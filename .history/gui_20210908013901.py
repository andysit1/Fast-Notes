...
#function that gets the link that the user inputs
def get_link():
    link = entry.get()

    if link == null:
        print('Please enter a link to a website.')
    ...
    for i in range(len(matching)):
        exec('Label%s=Label(window,text="%s", anchor="w", width=720, wraplengt=1080)\nLabel%d.pack()' % (i,matching[i],i))
